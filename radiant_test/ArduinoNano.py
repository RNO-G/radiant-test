...
class ArduinoNano():
    def __init__(self):
        try:
            self.dev = self.open_serial_port(port='/dev/ttyUSB0') # linux
            #self.dev = self.open_serial_port(port='/dev/cu.usbserial-AI02POV4') # mac
            #self.dev = self.open_serial_port(port='COM3') # windows

        except serial.SerialException as e:
            print(f"Arduino not connected: {e}, chose from this ports")
            print('\n'.join([p.device for p in list(list_ports.comports())]))

    def open_serial_port(self, port='/dev/ttyUSB0', baudrate=9600):
    ...
    def route_signal_to_channel(self, channel):
        try:
            line = self.routing(channel)
        except serial.SerialException as e:
            print(f"Caught SerialException: {e}, reconnect serial port")
