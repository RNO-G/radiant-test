...
def get_fit_results_str(data, ch, with_color=False):
    ...
    if data_ch["halfway"] is None:
        str_halfway = f'halfway: no fit result'
    else:
        str_halfway = f'halfway: {data_ch["halfway"]:.2f}'
    if data_ch["steepness"] is None:
        str_steepness =f'steepness: no fit result'
    else:
        str_steepness = f'steepness: {data_ch["steepness"]:.2f}'
...
def plot_channel(fig, ax, data, ch):
    ...
    snr_ch = ch_data['vpp_ch']

    thresh = ch_data["thresh"]
    ...
    ax.plot(np.asarray(thresh), trig_eff, marker='x', ls='',
            color='#2d5d7b', label=f'SNR: {snr_ch:.2f}', linewidth=1)
    ax.grid()

    if popt[0] is None or popt[1] is None:
        pass
    else:
        x_arr = np.linspace(np.min(thresh) - 0.1 * np.min(thresh), np.max(thresh) + 0.1 * np.max(thresh), 100)
...
def plot_ana(data):
    ...
    for i, ch in enumerate(deep_channels):
        thresh_1 = data['run']['measurements'][str(ch)]['measured_value']['thresh']
        trig_effs_1 = data['run']['measurements'][str(ch)]['measured_value']['trigger_effs']
        ax1.errorbar(thresh_1, trig_effs_1, fmt='x', color=colors[i], label=f'{ch}')
    ...
    for i, ch in enumerate(surface_channels):
        thresh_1 = data['run']['measurements'][str(ch)]['measured_value']['thresh']
        trig_effs_1 = data['run']['measurements'][str(ch)]['measured_value']['trigger_effs']
        ax2.errorbar(thresh_1, trig_effs_1, fmt='x', color=colors[i], label=f'{ch}')
        popt = [data['run']['measurements'][str(ch)]['measured_value']['fit_parameter']['halfway'], data['run']['measurements'][str(ch)]['measured_value']['fit_parameter']['steepness']]
        if popt[0] is None or popt[1] is None:
            pass
        else:
