...
def get_fit_results_str(data, ch, with_color=False):
    ...
    return (
        color_start
        + f"amp: {data_ch['fit_amplitude_lower_buffer']:6.1f} - "
        + f"freq: {data_ch['fit_frequency_lower_buffer']:6.2f} MHz - "
        + f"offset: {data_ch['fit_offset_lower_buffer']:6.2f} - "
        + f"avg. residual: {data_ch['fit_avg_residual_lower_buffer']:6.2f} - "
        + f"amp: {data_ch['fit_amplitude_higher_buffer']:6.1f} - "
        + f"freq: {data_ch['fit_frequency_higher_buffer']:6.2f} MHz - "
        + f"offset: {data_ch['fit_offset_higher_buffer']:6.2f} - "
        + f"avg. residual: {data_ch['fit_avg_residual_higher_buffer']:6.2f} - "
...
def plot_all(data, args_input="", args_channel=None, args_web=False):
    ...
    if not args_web:
        ...
        fname += f'_{config["args"]["frequency"]}MHz.pdf'
    ...
    fig2, axs = plt.subplots(nrows=nrows*2, ncols=ncols, sharex=True, figsize=(6 * ncols, 5 * nrows))
    idx = 0
...
def plot_channel(ax, data, ch, window_label, print_fit=False):
    data_ch = data["run"]["measurements"][f"{ch}"]["measured_value"]
...
def plot_single(data, ch):
    fig = plt.figure()
    ax = fig.subplots()
    plot_channel(ax, data, ch, window_label='', print_fit=True)
