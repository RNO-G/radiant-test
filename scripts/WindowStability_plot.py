...
def print_results(data):
    ...
    for ch, ele in measurements.items():        
        ...
        result_str += add_color(mean_variation < config["expected_values"]["variation_tolerance"]) + \
            f"{mean_variation:^20.2f}" + colorama.Style.RESET_ALL + " | "
            
        result_str += add_color(min_power > config["expected_values"]["min_power"]) + \
            f"{min_power:^20.2f}" + colorama.Style.RESET_ALL + " | "
            
        result_str += add_color(max_power < config["expected_values"]["max_power"]) + \
            f"{max_power:^20.2f}" + colorama.Style.RESET_ALL + "\n"
